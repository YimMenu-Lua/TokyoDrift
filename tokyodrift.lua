...
tokyodrift:add_imgui(function()
  ...
  if PED.IS_PED_IN_ANY_VEHICLE(self.get_ped(), true) then
    ...
    widgetToolTip(false, "Keeps your wheels turned when you exit your vehicle while holding [A] or [D].")
    ...
    ImGui.SameLine(); ImGui.Dummy(28, 1); ImGui.SameLine(); noJacking, _ = ImGui.Checkbox("Can't Touch This!", noJacking,
      true)
...
script.register_looped("game input", function()
  ...
  if PED.IS_PED_SITTING_IN_ANY_VEHICLE(self.get_ped()) then
    ...
    if holdF then
      if isDriving() and not is_typing and VEHICLE.IS_VEHICLE_STOPPED(self.get_veh()) then
...
script.register_looped("TDFT", function(script)
  ...
  if PED.IS_PED_SITTING_IN_ANY_VEHICLE(self.get_ped()) then
    ...
    if isDriving() and keepWheelsTurned and not holdF then
...
script.register_looped("Can't Touch This", function(ctt)
  if noJacking then
